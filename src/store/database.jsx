...
`,
  ...
  terms: `Thank you for your interest in Hipzone Automation India. We are committed to delivering innovative and reliable home automation solutions. Please review the following terms and conditions`,
  pay: `1. 25% – Upon order confirmation. 
  2. 50% – Upon marking automation points and providing wiring drawings at the site. 
  3. 75% – Upon completion of range mapping and site testing (including verification of electrical components, switch-boxes, lights, and automation provisions). 
  4. 100% – Prior to dispatch of materials.`,
  rate: { currency: "INR", rate: "5" },
  sale: {
    name: "Sales Team", contact: "name@hipzoneautomation", technumber: "+91 7012566008", techcontact: "support@hipzoneautomation.com"
  },
  pages: [],
};
...
export default function Database({ children }) {
  ...
  const saveChanges = useCallback(async () => {
    try {
      ...
      await localforage.setItem("hipzone-data", payload);
      ...
      try {
        ...
        await localforage.setItem("hipzone-data", fallbackPayload);
      } catch (e2) {
  ...
  useEffect(() => {
    (async () => {
      const stored = await localforage.getItem("hipzone-data");
      if (stored) {
        setHome(stored.home ?? initialState.home);
        setAbout(stored.about ?? initialState.about);
        setAddress(stored.address ?? initialState.address);
  ...
  // product fetch
  useEffect(() => {
    fetch("https://hipzonebackend.vercel.app/products")
      .then((r) => r.json())
      .then(setIsProduct)
      .catch(console.error);
  }, []);
  ...
  const resetData = async () => {
    await localforage.removeItem("hipzone-data");
