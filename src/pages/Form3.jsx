...
const OptimizedProductRow = memo(({
  ...
  // Auto-increment sno when row index changes
  useEffect(() => {
    if (table.type === "7col" && row.sno !== rowIndex + 1) {
      dispatch({ type: 'UPDATE_FIELD', field: 'sno', value: rowIndex + 1 });
      handleFieldCommit('sno', rowIndex + 1);
    }
  }, [rowIndex, table.type, row.sno]);
    ...
    <div className="bg-white rounded-lg border border-gray-200 p-6 relative">
      ...
      {table.type === "7col" && (
        <div className="space-y-4 pr-20">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                S.No <span className="text-green-600">(Auto)</span>
              </label>
              <FastInput
                type="number"
                value={row.sno}
                onChange={() => { }} // Read-only but shows current value
                className="w-full px-3 py-2 border border-gray-300 rounded-md bg-green-50 text-green-700 focus:outline-none font-medium"
                readOnly
              />
            </div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Description <span className="text-blue-600">(Fast Mode)</span>
            </label>
            <FastTextarea
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Tax Amount <span className="text-purple-600">(Auto)</span>
              </label>
              <input
                type="text"
      ...
      {table.type === "4col" && (
        <div className="space-y-4 pr-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Total <span className="text-purple-600">(Auto)</span>
              </label>
              <input
                type="text"
...
const VirtualizedTable = memo(({
  ...
  // Auto-renumber function for 7col tables
  const autoRenumberRows = useCallback(() => {
    if (table.type === "7col") {
      table.products.forEach((_, index) => {
        onUpdateRow(pageId, table.id, index, 'sno', index + 1);
      });
    }
  }, [table, pageId, onUpdateRow]);

  // Trigger renumbering when rows change
  useEffect(() => {
    autoRenumberRows();
  }, [table.products.length, autoRenumberRows]);
      ...
      <div className="bg-white px-4 py-3 border-b border-gray-200">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {table.type === "7col" && (
              <button
                onClick={autoRenumberRows}
                className="inline-flex items-center gap-1 px-3 py-1 bg-green-100 hover:bg-green-200 text-green-700 rounded-md text-sm font-medium transition-colors duration-200"
                title="Renumber all serial numbers"
              >
                #123
              </button>
            )}
      ...
      <div className="p-4" ref={containerRef}>
        ...
        <div className="mt-6 pt-4 border-t border-gray-200">
          ...
          >
            ...
            Add Product {table.type === "7col" && `(S.No ${table.products.length + 1})`}
          </button>
        </div>
          ...
          ? {
            ...
            tables: p.tables.map((t) =>
              t.id === tableId
                ? {
                  ...t,
                  products: t.products.filter((_, i) => i !== rowIndex),
                }
                : t
            ),
          }
        ...
        <main className='flex justify-center items-center'>
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-gray-900 mb-2 flex items-center gap-3">
              ...
              High-Performance Invoice Form
              <span className="text-lg bg-green-100 text-green-800 px-3 py-1 rounded-full font-medium">
                Optimized
              </span>
            </h1>
            <p className="text-gray-600">Ultra-fast input handling with auto-incrementing serial numbers</p>
          </div>
        </main>
        ...
        <div className="mb-6 p-4 bg-green-50 border border-green-200 rounded-lg">
          <div className="flex items-center gap-3">
            ...
            <span className="text-green-800 font-medium">
              Performance Mode Active - Fast Input Processing
            </span>
          </div>
        </div>
